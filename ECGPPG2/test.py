...
class ECGPPGCharacteristic(Characteristic):
    ...
    def get_ECGPPG_signal(self):
        ...
        print(DataStr[999])
        value=[]
        if self.count<(len(DataStr)/2):
            print(DataStr[self.count])
            a=DataStr[(self.count)*2]+"!"+DataStr[(self.count)*2+1]
            a=str(self.count)+":"+a+";"
            for c in a:
    ...
    def collectdata(self):
        ...
        for i in range(1000):
            ...
            ecg=Read_Data()
            ppg=PPGdata()
            f=str(round(clk,3))+','+str(round(ecg,5))+','+str(round(ppg,5))
            DataStr.append(f)
            ECG_LIST.append(round(ecg,5))
            PPG_LIST.append(round(ppg,5))
            TIME.append(round(clk,5))
            print(i)
            time.sleep(0.001)
        print(DataStr[999])
        with open('data_1000', 'w') as csvfile:
        	csvwriter = csv.writer(csvfile)
        	csvwriter.writerows(TIME)
        	csvwriter.writerows(ECG_LIST)
        return
